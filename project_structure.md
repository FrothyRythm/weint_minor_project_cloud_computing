...
- **backend/**: The Node.js Express server application.
  ...
  - **controllers/**: Business logic layer containing HTTP request-response handlers.
    - `authController.js`: Manages user registration, login, logout, and change password requests.
    ...
    - `shareController.js`: Manages generating temporary cryptographic share tokens, listing active shares, revoking shares, and resolving share tokens.
  ...
  - **routes/**: Maps Express router API endpoints to respective controller actions.
    - `auth.js`: Mounts `/register`, `/login`, `/logout`, and `/change-password`.
    - `files.js`: Mounts metadata list, download URL, deletion, upload URLs, and usage endpoints.
    - `share.js`: Mounts public resolution, protected creation, listing, and revocation endpoints.
...
- **frontend/**: Client browser interface application.
  - **public/**: Public assets.
    - **css/**: Style assets.
      - `styles.css`: Earthy color scheme styles, layout grids, sidebar configurations, search filters, and custom animations.
    - **js/**: Core Javascript functionalities.
      ...
      - `dashboard.js`: File list rendering, drag-and-drop file picker, S3 direct upload PUT handler, download link execution, modal dialogs, tab toggles, settings forms, and file previewers.
...
[
  {
    "id": "d3b07384-d113-4ec2-a5d6-c0c2be43000b",
    "file_id": 3,
    "expires_at": "2026-07-22T15:02:17.000Z",
    "view_count": 5,
    "filename": "my_document.pdf",
    "file_size": 1048576
  }
...
### 8. Revoke Share Link
- **Endpoint**: `DELETE /api/shares/:token` (e.g. `/api/shares/d3b07384-d113-4ec2-a5d6-c0c2be43000b`)
- **Response (200 OK)**:
```json
{
